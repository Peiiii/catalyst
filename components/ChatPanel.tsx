...
const CodeBlock: React.FC<{ code: { language: string, content: string } }> = ({ code }) => {
    ...
    return (
        <div className="bg-slate-900/50 rounded-md mt-2 border border-slate-600">
            ...
            {isExpanded && (
                <pre className="p-3 text-xs text-slate-300 overflow-x-auto bg-slate-900 rounded-b-md">
...
const ChatPanel: React.FC<ChatPanelProps> = ({ messages, onSendMessage, isThinking }) => {
  ...
  return (
    <div className="w-full md:w-1/3 xl:w-1/4 bg-slate-800/80 border-r border-slate-700 flex flex-col h-full">
      <h2 className="text-xl font-bold text-slate-200 p-4 border-b border-slate-700 flex-shrink-0">AI Agent</h2>
      <div className="flex-1 p-4 overflow-y-auto">
        <div className="flex flex-col space-y-4">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex items-start ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`rounded-lg px-4 py-2 max-w-xs lg:max-w-sm ${
                msg.sender === 'user'
